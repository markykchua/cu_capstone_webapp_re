...
public class CookieRelation : IFlowRelation
{
    ...
    public void InsertRelation(UserFlow flow)
    {
        ...
        // foreach (var pair in cookiePairings)
        // {
        //     cookieCount++;
        //     Console.WriteLine($"Found cookie ({pair.Key.Request.Url}) with {pair.Value.Count} uses");
        //     pair.Key.AddExport($"extracted_cookie_token_{cookieCount}", new Export("$.Response.Headers.Set-Cookie"));
        //     foreach (FlowElement element in pair.Value)
        //     {
        //         element.Request.Headers["cookie"] = $"{{{{extracted_cookie_token_{cookieCount}}}}}";
        //     }
        // }
